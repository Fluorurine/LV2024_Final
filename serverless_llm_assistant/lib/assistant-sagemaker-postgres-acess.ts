...
interface SageMakerRdsAccessConstructProps {
	vpc: ec2.Vpc;
	rdsCluster: rds.DatabaseInstance;
}
...
export class SageMakerRdsAccessConstruct extends Construct {
	...
	constructor(
		scope: Construct,
		id: string,
		props: SageMakerRdsAccessConstructProps
	) {
		super(scope, id);

		// Create a security group to allow access to the DB from a SageMaker processing job
		this.processingSecurityGroup = new ec2.SecurityGroup(
			this,
		...
		this.processingSecurityGroupIdParameter = new ssm.StringParameter(
			this,
		...
		this.dbSecretArnParameter = new ssm.StringParameter(
			this,
		...
		this.subnetIdsParameter = new ssm.StringParameter(
			this,
			"SubnetIdsParameter",
			{
